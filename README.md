...
You can install Helm repos and CRDs before the main Kustomization scripts by adding the helm charts to the `extra-manifests-preinstall` folder and specifying them there in kustomization.yaml.tpl, just like with `extra-manifests`.
Also, if you need to wait for the CRDs to be ready, you can specify additional wait commands to `extra_kustomize_pre_install_post_deploy_commands`, which defaults to `sleep 10`.
